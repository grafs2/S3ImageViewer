...
- **Cache aufbauen** — full rebuild: downloads and re-generates all thumbnails from scratch
- **Delta Sync** — incremental update: only downloads new images and removes thumbnails whose originals no longer exist on S3
- **DB Stats** — view detailed insights about the local cache, including database size, image counts, and average thumbnail size
...
On first run, click **"Cache aufbauen"** in the top bar. The app scans your S3 bucket, downloads all images, and generates thumbnails stored locally in `cache.db`. This may take a few minutes depending on the number of images. The page refreshes automatically when done.
...
| Button | When to use |
|---|---|
| **Delta Sync** | Regularly — adds new images, removes deleted ones. Fast. |
| **Cache aufbauen** | When thumbnails look wrong or after major bucket changes. Slow. |
| **DB Stats** | To check cache health, storage usage, and synchronization timestamps. |
